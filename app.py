...
SCORE: [0.0-1.0]"""
    ...
    def create_synthetic_ground_truth(self, df: pd.DataFrame, sample_size: int = 100) -> pd.DataFrame:
        """Create synthetic ground truth labels for evaluation (heuristic-based)"""
        ...
        if content_column is None:
            st.error("No content column found for ground truth generation")
            return sample_df
        ...
        for idx, row in sample_df.iterrows():
            ...
            # Age group heuristics based on keywords
            if any(word in text for word in ['school', 'homework', 'class', 'teacher', 'omg', 'literally', 'teen']):
                sample_df.at[idx, 'true_age_group'] = 'teens'
            elif any(word in text for word in ['college', 'job', 'career', 'interview', 'apartment', 'university']):
                sample_df.at[idx, 'true_age_group'] = 'young_adults'
            elif any(word in text for word in ['kids', 'family', 'mortgage', 'parenting', 'work', 'children']):
                sample_df.at[idx, 'true_age_group'] = 'adults'
            elif any(word in text for word in ['retirement', 'grandchildren', 'health', 'doctor', 'retired']):
                sample_df.at[idx, 'true_age_group'] = 'seniors'
            else:
                sample_df.at[idx, 'true_age_group'] = 'young_adults'  # default
            
            # Confidence level heuristics
            if any(word in text for word in ['not good', 'terrible', 'awful', 'dont know', 'scared', 'worried', 'confused', 'lost']):
                sample_df.at[idx, 'true_confidence_level'] = 'low'
            elif any(word in text for word in ['confident', 'sure', 'excellent', 'great', 'amazing', 'proud', 'definitely']):
    ...
    def calculate_evaluation_metrics(self, results_df: pd.DataFrame, ground_truth_df: pd.DataFrame) -> Dict:
        """Calculate evaluation metrics without sklearn"""
        
        # Find common identifier
        id_column = None
        for col in ['post_id', 'Post ID', 'original_index', 'index']:
            if col in results_df.columns and col in ground_truth_df.columns:
                id_column = col
                break
        
        if id_column is None:
            # If no common ID, assume same order
            if len(results_df) != len(ground_truth_df):
                return {'error': 'Cannot match results with ground truth - no common identifier and different lengths'}
            
            # Create temporary index for matching
            results_df = results_df.reset_index(drop=True)
            ground_truth_df = ground_truth_df.reset_index(drop=True)
            results_df['temp_index'] = range(len(results_df))
            ground_truth_df['temp_index'] = range(len(ground_truth_df))
            id_column = 'temp_index'
        
        # Merge datasets
        try:
            merged_df = results_df.merge(
                ground_truth_df[[id_column, 'true_age_group', 'true_confidence_level']], 
                on=id_column, 
                how='inner'
            )
        except Exception as e:
            return {'error': f'Failed to merge datasets: {str(e)}'}
        
        if len(merged_df) == 0:
            return {'error': 'No matching records found between results and ground truth'}
        
        # Calculate simple accuracy metrics
        age_correct = (merged_df['age_group'] == merged_df['true_age_group']).sum()
        conf_correct = (merged_df['confidence_level'] == merged_df['true_confidence_level']).sum()
        total = len(merged_df)
        ...
        age_class_accuracy = {}
        for age in age_groups:
            age_subset = merged_df[merged_df['true_age_group'] == age]
            if len(age_subset) > 0:
                correct = (age_subset['age_group'] == age_subset['true_age_group']).sum()
                age_class_accuracy[age] = correct / len(age_subset)
            else:
                age_class_accuracy[age] = 0
        
        conf_class_accuracy = {}
        for conf in conf_levels:
            conf_subset = merged_df[merged_df['true_confidence_level'] == conf]
            if len(conf_subset) > 0:
                correct = (conf_subset['confidence_level'] == conf_subset['true_confidence_level']).sum()
                conf_class_accuracy[conf] = correct / len(conf_subset)
            else:
                conf_class_accuracy[conf] = 0
        
        # Create confusion matrix data
        age_confusion = {}
        for true_age in age_groups:
            age_confusion[true_age] = {}
            true_subset = merged_df[merged_df['true_age_group'] == true_age]
            for pred_age in age_groups:
                count = (true_subset['age_group'] == pred_age).sum()
                age_confusion[true_age][pred_age] = count
        
        conf_confusion = {}
        for true_conf in conf_levels:
            conf_confusion[true_conf] = {}
            true_subset = merged_df[merged_df['true_confidence_level'] == true_conf]
            for pred_conf in conf_levels:
                count = (true_subset['confidence_level'] == pred_conf).sum()
                conf_confusion[true_conf][pred_conf] = count
        
        return {
            'sample_size': total,
            'age_accuracy': age_accuracy,
            'confidence_accuracy': conf_accuracy,
            'age_class_accuracy': age_class_accuracy,
            'confidence_class_accuracy': conf_class_accuracy,
            'age_confusion_matrix': age_confusion,
            'confidence_confusion_matrix': conf_confusion,
...
def main():
    ...
    # Main tabs
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Dataset", "🎯 Analysis", "📈 Results", "🔬 Evaluation", "ℹ️ About"])
